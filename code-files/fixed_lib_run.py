...
def run_lib(afglms_file_name: str, wlen_min: float, wlen_max: float, 
    ...
    for idx, conf_obj in enumerate(conf_obj_list):
        ini_file_name = f"{afglms_file_name}_conf_obj{str(idx+1)}_sza{sza}_phi0{phi0}_alt{alt}_tau{tau}.ini"
    ...
    def save_output_params(results):
        ...
        output_file_name = f"{afglms_file_name}_sza{sza}_phi0{phi0}_alt{alt}_tau{tau}.json"
