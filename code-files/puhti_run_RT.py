...
def run_RT(start_idx):
    ...
    sza_range = [30, 35]
    alt_range = [0, 1.5]
    tau_range = [0.05, 0.15, 0.3]
    
    # Possibility to vary (if needed)
    vza = 0
    phi0 = 0
    phi = 0
        ...
        for sza in sza_range:
            for alt in alt_range:
                for tau in tau_range:
                    # Exclude the combination sza=35, alt=0, tau=0.1
                    if not (sza == 35 and alt == 0 and tau == 0.15):
                        run_lib(file, 
                                wlen_min, 
                                wlen_max, 
                                phi, 
...
# Note that we have already produced the output files in Puhti where
# sza=35, alt=0, and tau=0.15, so we don't want to include that combination
# into the new round of runs in Puhti.
